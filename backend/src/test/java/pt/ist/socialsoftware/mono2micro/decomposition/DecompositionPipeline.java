...
interface DecompositionPipeline {
    ...
    /** Step 2. Uploads the two files the {@code Accesses Based} group requires. */
    void addAccessesRepresentations(String codebaseName, byte[] idToEntity, byte[] accesses);

    /** Step 3. Returns the strategy name, which is deterministic: {@code "<codebase> - A Strategy"}. */
    String createStrategy(String codebaseName);

    /** Step 4. Returns the generated similarity name, read back from the strategy. */
    String createSimilarity(String strategyName);
